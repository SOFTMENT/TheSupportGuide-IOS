...
class BusinessProfileViewController : UIViewController {
    ...
    @IBOutlet weak var editBusinessProfile: UIView!
    @IBOutlet weak var editOwnerProfile: UIView!
    ...
    override func viewDidLoad() {
        ...
        profilePic.layer.cornerRadius = 8
         if let path = B2BModel.data!.image, !path.isEmpty {
            profilePic.sd_setImage(with: URL(string: path), placeholderImage: UIImage(named: "profile-placeholder"))
        }
        
        mName.text = B2BModel.data!.name ?? ""
        mEmail.text = B2BModel.data!.email ?? ""
        ...
        editOwnerProfile.isUserInteractionEnabled = true
        editOwnerProfile.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(editOwnerProfileclicked)))
        
        editBusinessProfile.isUserInteractionEnabled = true
        editBusinessProfile.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(editBusinessProfileClicked)))
    ...
    @objc func editOwnerProfileclicked(){
        performSegue(withIdentifier: "editOwnerProfileSeg", sender: nil)
    }
    
    @objc func editBusinessProfileClicked(){
        performSegue(withIdentifier: "editB2bSeg", sender: nil)
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "editB2bSeg" {
