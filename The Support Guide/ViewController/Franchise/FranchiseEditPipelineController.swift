//
//  FranchiseEditPipelineController.swift
//  The Support Guide
//
...
class FranchiseEditPipelineController : UIViewController {
...
var piplineModel : PipelineModel?
override func viewDidLoad() {
    
    if let pipeLineModel = piplineModel {
        ...
        if pipeLineModel.type == "b2b" {
            self.businessCheck.isSelected = true
        }
        else {
            self.fundraiserCheck.isSelected = true
        }
        
        intrestLevel.text = "\(pipeLineModel.level ?? 1)"
...
@IBAction func addBtnClicked(_ sender: Any) {
    ...
    if sName == "" {
        ...
        if self.franchiseType == .B2B {
            self.piplineModel!.type = "b2b"
        }
        else if self.franchiseType == .B2B {
            self.piplineModel!.type = "sales"
        }
        self.piplineModel!.aboutBusiness = sAbout
        self.piplineModel!.address = self.fAddress.text
        self.piplineModel!.createDate = Date()
        self.piplineModel!.email = sEmail
        
        self.piplineModel!.latitude = self.latitude
        self.piplineModel!.longitude = self.longitude
        self.piplineModel!.name = sName
        self.piplineModel!.openingTime = self.openingDatePicker.date
        self.piplineModel!.closingTime = self.closingDatePicker.date
        self.piplineModel!.phoneNumber = sPhoneNumber
        self.piplineModel!.franchiseId = FranchiseModel.data!.uid
        self.piplineModel!.level = self.intrestLevelNumber
        let location = CLLocationCoordinate2D(latitude: self.latitude, longitude: self.longitude)
        let hash = GFUtils.geoHash(forLocation: location)
        self.piplineModel!.geoHash = hash
        
        self.ProgressHUDShow(text: "")
        
        if isImageSelected {
            self.uploadImageOnFirebase(id: self.piplineModel!.id ?? "123") { downloadURL in
                self.piplineModel!.image = downloadURL
                self.updatePipeline(pipeLineModel: self.piplineModel!)
...
func updatePipeline(pipeLineModel : PipelineModel){
    try? FirebaseStoreManager.db.collection("Franchises").document(pipeLineModel.franchiseId ?? "123")
        .collection("Pipelines").document(pipeLineModel.id ?? "123").setData(from: pipeLineModel,merge : true,completion: { error in
        self.ProgressHUDHide()
        if let error = error {
            self.showError(error.localizedDescription)
        }
        else {
            self.showToast(message: "Pipeline Updated")
            let seconds = 2.5
        ...
        self.present(image,animated: true)
                                                                                                                                                    ...
                                                                                                                                                    extension FranchiseEditPipelineController : UIImagePickerControllerDelegate, UINavigationControllerDelegate,CropViewControllerDelegate {
            ...
            storage.putData(uploadData, metadata: nil) { (metadata, error) in
                ...
                if error == nil {
                    ...
                    completion(downloadUrl)
                                                                                                                                                    ...
                                                                                                                                                    extension FranchiseEditPipelineController : UITableViewDelegate, UITableViewDataSource {
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            return Google_Places_Cell()
                                                                                                                                                    ...
                                                                                                                                                    extension FranchiseEditPipelineController : UITextFieldDelegate {
        ...
        func textFieldShouldReturn(_ textField: UITextField) -> Bool {
            ...
            return true
                                                                                                                                                    ...
                                                                                                                                                    extension FranchiseEditPipelineController : UIPickerViewDelegate, UIPickerViewDataSource {
