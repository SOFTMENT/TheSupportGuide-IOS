//
//  SalesEditMemberViewController.swift
//  The Support Guide
//
...
class SalesEditMemberViewController : UIViewController {
    ...
    @IBAction func addBtnClicked(_ sender: Any) {
        ...
        let sName = mName.text
         if sName == "" {
            ...
            self.memberModel!.name = sName
      
            if isImageSelected {
                self.uploadImageOnFirebase(uid: self.memberModel!.id ?? "123") { downloadURL in
                    self.memberModel!.profilePic = downloadURL
                    self.updateMember()
                }
    ...
    func updateMember(){
        ...
        try? FirebaseStoreManager.db.collection("Fundraisers").document(memberModel!.fundraiserId ?? "123").collection("Members").document(memberModel!.id ?? "123").setData(from: memberModel!,merge : true) { error in
            ...
            if let error = error {
                ...
                self.showToast(message: "Member Updated")
                let seconds = 2.5
...
extension SalesEditMemberViewController : UITextFieldDelegate {
...
extension SalesEditMemberViewController : UIImagePickerControllerDelegate, UINavigationControllerDelegate,CropViewControllerDelegate {
