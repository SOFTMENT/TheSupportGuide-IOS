//
//  SalesProfileViewController.swift
//  The Support Guide
//
...
class SalesProfileViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        mName.text = FundraiserModel.data!.name ?? ""
        mEmail.text = FundraiserModel.data!.email ?? ""
    ...
    @IBAction func editBtnClicked(_ sender: Any) {
        performSegue(withIdentifier: "editFundrraiserSeg", sender: nil)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "editFundrraiserSeg" {
            if let VC = segue.destination as? FranchiseEditFundraiserViewController {
                VC.fundraiserModel = FundraiserModel.data
            }
        }
    }
