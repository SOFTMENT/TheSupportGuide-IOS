//
//  AddFranchiseViewController.swift
//  The Support Guide
//
...
class AddFranchiseViewController : UIViewController {
    ...
    @IBAction func addBtnClicked(_ sender: Any) {
        ...
        if !isImageSelected {
            self.showToast(message: "Uplaod Franchise Image")
        }
        else if sName == "" {
            ...
            self.createAuthUser(name: sName!, email: sEmail!, password: sPassword!, isAdmin: true) { uid, error in
                ...
                if let error = error {
                    ...
                    if let uid = uid {
                        ...
                        franchiseModel.uid = uid
                        franchiseModel.about = sAbout
                        franchiseModel.address = self.fAddress.text
                        franchiseModel.createDate = Date()
                        franchiseModel.email = sEmail
                        franchiseModel.password = sPassword
                        franchiseModel.latitude = self.latitude
                        franchiseModel.longitude = self.longitude
                        franchiseModel.name = sName
                        let location = CLLocationCoordinate2D(latitude: self.latitude, longitude: self.longitude)
                        let hash = GFUtils.geoHash(forLocation: location)
                        franchiseModel.geoHash = hash
                        
                        self.uploadImageOnFirebase(id: uid) { downloadURL in
                            franchiseModel.image = downloadURL
                            self.addFranchise(franchiseModel: franchiseModel) { error in
                                self.ProgressHUDHide()
                                if let error = error {
                                    self.showError(error)
                                }
                                else {
                                    self.performSegue(withIdentifier: "copySeg", sender: franchiseModel)
                                }
                            }
                        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "copySeg" {
            if let VC = segue.destination as? CopyEmailPasswordViewController {
...
extension AddFranchiseViewController : UIImagePickerControllerDelegate, UINavigationControllerDelegate,CropViewControllerDelegate {
...
extension AddFranchiseViewController : UITableViewDelegate, UITableViewDataSource {
...
extension AddFranchiseViewController : UITextFieldDelegate {
